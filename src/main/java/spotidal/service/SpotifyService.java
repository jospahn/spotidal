...
import spotidal.model.ExportData;
import spotidal.model.Playlist;
...
public class SpotifyService {
    ...
    private SpotifyApi api() {
        String accessToken = authService.checkAndRefresh().orElseThrow().accessToken();
    ...
    public String getCurrentUserId() throws Exception {
        User user = api().getCurrentUsersProfile().build().execute();
        return user.getId();
    }
    ...
    public void exportLikedSongs(String outputFile) throws Exception {
        ...
        while (true) {
            GetUsersSavedTracksRequest request = api()
                    .getUsersSavedTracks()
                    .limit(limit)
                    .offset(offset)
                    .build();
    ...
    public void exportPlaylists(String outputFile) throws Exception {
        ...
        while (true) {
            GetListOfCurrentUsersPlaylistsRequest request = api()
    ...
    private Playlist loadFullPlaylist(String playlistId) throws Exception {
        se.michaelthelin.spotify.model_objects.specification.Playlist spotifyPlaylist =
                api().getPlaylist(playlistId).build().execute();
